...
const Services = () => {
    return (
        <div>
              <h1 className="text-primary font-semibold text-3xl mt-14">Proficiencies and Skills:</h1>
              <div className='grid md:grid-cols-3 my-8 gap-5 '>
              {
                services.map(service => <div key={service.id} className='service-card flex flex-col justify-center items-center p-6 text-center text-white'>
                    <img src={service.image} alt="" className='mb-3' height={"70px"} width={"70px"} />
                    <h6 className='text-2xl mb-4'>{service.name}</h6>
                    <p className='text-base text-justify'text-justify>{service.description}</p>
                </div>)
              }
              </div>
        </div>
